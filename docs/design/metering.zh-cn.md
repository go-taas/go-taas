...
> **`billing.settlements` 上的增量消费者（issue #7）。** 该主题可承载多个消费者。特性 #5 的计费消费者将每个 key-小时计价为 `charge_records`；一条免费的精确结算轨（Nano / XNO）可以在同一主题上注册为*额外*消费者，读取相同的事件负载（用量记录 id、key、组织、周期、token 求和——D6：不含金额），并以原始精度结算已计价金额。这绝不改变计量事件契约或负载；只是新增一个消费者。见 pricing.md §1.4.1。
