...
> **Additive consumers on `billing.settlements` (issue #7).** The subject may carry more than one consumer. Feature #5's billing consumer prices each key-hour into `charge_records`; a feeless exact-settlement leg (Nano / XNO) may register as an *additional* consumer on the same subject, reading the same event payload (usage record id, key, org, period, token sums — D6: no amounts) and settling the priced amount at raw precision. This never changes the metering event contract or payload; it only adds a consumer. See pricing.md §1.4.1.
